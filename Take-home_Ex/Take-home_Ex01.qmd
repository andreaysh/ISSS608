...
pacman::p_load(tidyverse, haven, knitr,
               ...
               ggdist, ggtext, ggalt,
               cowplot, ggnewscale,FunnelPlotR)
```
...
From the above analysis, it suggests that age alone may not be the strongest predictor of heart attack risk, other demographic, lifestyle or health factors might play a key role.

In the following analysis, we will examine the strongest predictors of heart attack incidents based on three key factor types: **Demographic, Lifestyle, and Medical/Health.**
...
+-----------------+-------------------------------------------------------------------------------------------------------+
| Lifestyle       | Smoking_History, Alcohol_Consumption, Physical_Activity, Diet_Quality, Stress_Levels_Category         |
+-----------------+-------------------------------------------------------------------------------------------------------+
| Medical/ Health | Diabetes_History, Hypertension_History, Cholesterol_Level, BMI, Heart_Rate, Systolic_BP, Diastolic_BP |
+-----------------+-------------------------------------------------------------------------------------------------------+
...
heart_attack_long <- heart_attack_2 %>%
  ...
  filter(Heart_Attack_Occurrence == "Yes") %>%  # 🔹 Keep only heart attack cases
...
#### 4.3.2 Does health factors play a role in contributing to heart attack incidents

##### 4.3.2.1 Correlation matrix
...
library(ggplot2)
library(stats)
# library(glmnet) # Removed since it's not installed
...
# Convert Heart_Attack_Occurrence to binary factor
relevant_factors$Heart_Attack_Occurrence <- factor(ifelse(relevant_factors$Heart_Attack_Occurrence == "Yes", 1, 0), levels = c(0, 1))
