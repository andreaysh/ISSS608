...
In the code below, we will fit four models:
-   Error-Trend-Season (ETS) model by using `exp_smoothing()`
-   Auto ARIMA model by using `arima_reg()`
-   Boosted Auto ARIMA by using `arima_boost()`
-   Prophet model by using `prophet_reg()`
...
#### 5.4.7 Examine the model accuract metrics after refitting the full data set
